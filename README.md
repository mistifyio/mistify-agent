...
Mistify Agent is the "core" agent. It provides a REST-ish API for manipulating a
hypervisor and defines a general set of actions, but the majority of the work is
done by sub-agents. Sub-agents are assumed to be running on the same host as the
agent and communication is done via JSON-RPC over HTTP.
...
to complete the action, configurable in the config file. All steps must succeed,
in order, for an action to be considered successful.
